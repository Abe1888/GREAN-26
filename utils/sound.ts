...
function playMorningSunshineSound(): void {
...
function getSoundStatus(): { [key: string]: string } {
  return process.env.NODE_ENV === 'development' ? { ...soundLoadStatus } : {};
}

// Test individual sound file loading
async function testSoundFile(filename: string): Promise<boolean> {
...
function setAudioVolume(volume: number): void {
  const preferences = getAudioPreferences();
